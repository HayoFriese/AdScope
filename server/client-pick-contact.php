<?php
	include "db.php";
	$cid = isset($_REQUEST['id']) ? $_REQUEST['id']:null;
				...
				if($cid != "" || isset($cid) || $cid != null){
							...
							<div id=\"select-contact\">";
								...
								$sqlFirst = "SELECT * FROM contact 
									WHERE NOT EXISTS (SELECT 1 FROM relatedcontact WHERE contact.idcontact = relatedcontact.contact AND contact.client = $cid) 
									AND (contact.client IS NULL OR contact.client = 0);";
								$rFirst = mysqli_query($conn, $sqlFirst) or die(mysqli_error($conn));
								if(mysqli_num_rows($rFirst) != 0){
									$sqlcontact = "SELECT * FROM contact WHERE contact.client IS NULL OR contact.client = 0";
									...
									while($row = mysqli_fetch_assoc($rcontact)){
										...
										$sqlcheck = "SELECT * FROM relatedcontact WHERE relatedcontact.contact = $idcontact AND relatedcontact.client = $cid";
